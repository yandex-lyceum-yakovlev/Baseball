...
public class GameScreen extends ScreenAdapter {
    ...
    public GameScreen(Tennis tennis) {
        ...
        backgroundTexture = tennis.textureManager.backgroundImg;
...
//        renderer.render(tennis.world, tennis.camera.combined.cpy().scl(1 / SCALE));
    ...
    public void nextRound() {
        ...
        ballObject = new BallObject(tennis.textureManager.ballImg, ballX, ballY, BALL_DIAMETER, tennis.world);

        float racketX = SCREEN_WIDTH / 2f;
        float racketY = SCREEN_HEIGHT * 0.1f;
        racketObject = new RacketObject(tennis.textureManager.racketImg, racketX, racketY, RACKET_WIDTH, RACKET_HEIGHT, tennis.world);

        float targetX = MathUtils.random(100, SCREEN_WIDTH - 100);
        float targetY = MathUtils.random(SCREEN_HEIGHT * 2f / 3f, SCREEN_HEIGHT - 100);
        targetObject = new TargetObject(tennis.textureManager.targetImg, targetX, targetY, TARGET_DIAMETER, tennis.world);

        for (int i = 0; i < score; i++) {
            float x = MathUtils.random(100, SCREEN_WIDTH - 100);
            float y = MathUtils.random(SCREEN_HEIGHT * 2f / 3f, SCREEN_HEIGHT - 100);

            ObstacleObject obstacle = new ObstacleObject(tennis.textureManager.obstacleImg, x, y, OBSTACLE_DIAMETER, tennis.world);
    ...
    private void showGameOverUI() {
        ...
        menuButton = new ButtonView(
            tennis.textureManager.buttonImg,
            SCREEN_WIDTH / 2f - 100,
            SCREEN_HEIGHT / 2f - 40,
            200, 50,
            tennis.largeWhiteFont,
